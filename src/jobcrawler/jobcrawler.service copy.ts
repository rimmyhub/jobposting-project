...
export class JobcrawlerService {
  ...
  async crawlJobs() {
    ...
    while (i <= 1) {
      ...
      const jobInfo = this.jobParsing(jobpostingContent);
      ...
      for (const job of jobInfo) {
        ...
        if (!existingCompany) {
          ...
          companyInfo = this.companyParsing(companyContent, job.companyId); // 추가 데이터 추가
